...
class FlashMHAv2(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, bias=True, batch_first=True, attention_dropout=0.0,
        ...
        if bias:
            ...
            self.register_parameter('in_proj_bias', None)
        self.inner_attn = FlashAttentionv2(attention_dropout=attention_dropout, **factory_kwargs)
        self.out_proj = nn.Linear(embed_dim, embed_dim, bias=bias)
        self._reset_parameters()
    ...
    def forward(self, q, k, v, attn_mask=None, key_padding_mask=None):
        ...
        v = rearrange(v, 'b s (h d) -> b s h d', h=self.num_heads)
        kv = torch.stack([k, v], dim=2)
        context, attn_weights = self.inner_attn(q, kv, attn_mask=attn_mask, key_padding_mask=key_padding_mask, causal=self.causal)
        return self.out_proj(rearrange(context, 'b s h d -> b s (h d)')), attn_weights